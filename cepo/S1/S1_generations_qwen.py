...
logging.basicConfig(
    filename=os.path.join(LOG_DIR, "s1_low_success_8011.log"),
    level=logging.INFO,
...
async def generate_plans(dataset, indices, problem_key, answer_key):
    async def process_sample(index, sample):
        ...
        cb_log['question'] = sample[problem_key] if sample[problem_key] is not None else ""
