...
logging.basicConfig(
    filename=os.path.join(LOG_DIR, "s1_log_non_verifiable.log"),
    level=logging.INFO,
...
# Define directories
BASE_DIR = "/mlf-transfers-only/harshg1/inference-time-effort/CePO/non-verifiable_datasets/Magpie_reasoning_v1/"
DIRS = {
    ...
    "buckets": os.path.join(BASE_DIR, "buckets3"),
}
...
def get_client(server='local_server'):
    if server == 'local_server':
        api_key = "serving-on-vllm"
        model = "meta-llama/Llama-3.3-70B-Instruct"
        return (Cerebras(api_key=api_key, base_url="http://127.0.0.1:8066/") , model)
        # model = "/home/ivanl/mlf2/magpieskyt1dsthinkv2cepos1v2s2_llama3p3_70b_ftrev2/"
        # return (Cerebras(api_key=api_key, base_url="http://127.0.0.1:8044/"), model)
    elif server == 'remote_server':
        api_key = "serving-on-vllm"
        model = "meta-llama/Llama-3.3-70B-Instruct"
        return (Cerebras(api_key=api_key, base_url="http://127.0.0.1:9000/"), model)
        # model = "/home/ivanl/mlf2/magpieskyt1dsthinkv2cepos1v2s2_llama3p3_70b_ftrev2/"
        # return (Cerebras(api_key=api_key, base_url="http://127.0.0.1:9800/") , model)
    elif server== 'gemini':
        base_url = "https://generativelanguage.googleapis.com/v1beta/openai/"
        model = "gemini-2.0-flash-exp"
        api_key = "AIzaSyCVdeVHAIdhJnMJj7ONYMyO3P3jBGXLNLo"
        return (AsyncOpenAI(api_key=api_key, base_url=base_url) , model)
        

##### clients #####
base_client , base_model = get_client('local_server')
client , model = get_client('remote_server')
gemini_client , gemini_model = get_client('gemini')
...
async def call_to_gemini(messages):
    """Handles API calls with retry logic"""
    MAX_RETRIES = 2
    RETRY_DELAY = 60  # seconds
    logging.info(f"Calling VLLM with prompt")
    for attempt in range(MAX_RETRIES):
        try:
            async with semaphore:
...
def get_dataset(dataset_name="AI-MO/NuminaMath-1.5", num_samples=10500):
...
def get_dataset_magpie_reasoning(dataset_name="Magpie-Align/Magpie-Reasoning-V1-150K",num_samples=10500):
    reasoning_train = load_dataset(dataset_name, split="train")
    logging.info(f"Loaded dataset with {len(reasoning_train)} samples")
    reasoning_train = reasoning_train.shuffle(seed=42).select(range(min(num_samples, len(reasoning_train))))
    return (reasoning_train, "instruction", "response")
...
async def compare_code_responses(question: str, response1: str, response2: str) -> dict:
    """Compares two code responses using an AI model asynchronously."""
    llm_judge_prompt = ("""
        Given two different responses to a coding question, please analyze them based on the following criteria:
            Correctness: Does the code achieve the stated objective? Are there any logical errors or edge cases not handled?
            Completeness: Does it address all aspects of the problem requirements? Are important considerations like error handling, input validation, and edge cases covered?
            Efficiency: Compare the algorithmic complexity and resource usage. Are there unnecessary operations or redundant code?
            Readability and Maintainability:
            Code structure and organization
            Variable/function naming clarity
            Comments and documentation quality
            Adherence to language-specific conventions and best practices
            Extensibility: How easily can the code be modified or expanded for future requirements?
            For each criterion, explain your reasoning and provide specific examples from the responses.
            Then, classify the overall comparison into one of these categories:
            Approximately Equal: The solutions are of similar quality with only minor differences that don't significantly impact their effectiveness (e.g. Approximately Equal)
            Marginally Better: One solution has noticeable advantages but the differences are not dramatic (e.g., slightly better error handling or marginally more efficient) e.g. (Marginally Better ; R1 marginally better than R2 )
            Significantly Better: One solution is clearly superior with substantial advantages in multiple criteria (e.g., much better performance, handles important edge cases the other misses, or significantly more maintainable) (e.g., Significantly Better ; R1 is significantly better than R2)
            Provide your classification with specific reasoning for why it falls into that category.
                        """)
    ...
    try:
        response = await call_to_gemini(messages)
        if response is None:
            raise ValueError("Received empty response from LLM.")

        classification = re.search(r"(Approximately Equal|Marginally Better|Significantly Better)", response)
        reasoning = response.split("\n\n")[-1]
        classification_tag = classification.group(0) if classification else "Unknown"
        if "R2 is significantly better than R1" in response: is_correct = True 
        else: is_correct = False
        
        logging.info(f"Classification tag: {classification_tag} , is_correct: {is_correct}")

        return {
            "comparison_llm_judge": response,
            "classification": classification_tag,
            "reasoning": reasoning,
            "is_correct": is_correct
        }    
    except Exception as e:
        logging.error(f"Error in compare_code_responses: {e}")
        return None



async def answer_process_data(data,index, datatype='verifiable'):
    ...
    try:
        if os.path.exists(f"{DIRS['analysed']}/analysed_data_{index}.csv"):
            ...
            elif datatype == 'non-verifiable':
                # Extract answers in parallel
                tasks = [compare_code_responses(entry["question"], entry["answer"], entry["execution"]) for entry in data]

                results = await rate_limited_gather(tasks, desc="Comparing code responses")
                # Process results and handle any exceptions
                for entry, result in zip(data, results):
                    if result is None:
                        print(f"Error in comparing code responses for problem {entry['problem_id']}")
                        continue

                    entry["comparison_llm_judge"] = result["comparison_llm_judge"]
                    entry["classification"] = result["classification"]
                    entry["reasoning"] = result["reasoning"]
                    entry["is_correct"] = result["is_correct"]
...
def save_buckets(data_grouped):
    ...
    try:
        for index, row in df_grouped.iterrows():
            ...
            if os.path.exists(output_file):
                with open(output_file, 'r') as f:
                    ...
                    if any(e['problem'] == entry['problem'] and e['plan_id'] == entry['plan_id'] for e in existing_entries):
                        logging.info(f"Entry for problem {row['problem_id']} and plan {row['plan_id']} already exists. Skipping.")
                        continue
        ...
        logging.error(f"Error in save_buckets: {str(e)}")
...
async def process_batch(dataset, indices, problem_key, answer_key):
    ...
    for coro in tqdm(asyncio.as_completed(tasks), total=len(tasks)):
        ...
        ### analysed file_path
        data_grouped, analysed_file_path = await answer_process_data(executed_plan, executed_plan[0]['problem_id'],datatype='non-verifiable')
        logging.info(f"analysed Done for index ")
        
        ### getting data into buckets ###
        save_buckets(data_grouped)
        logging.info(f"buckets Done for index ")
